...
Date: 2026-07-29
Status: 29 个独立 Capsule 已实现、测试，并写入受控性能权重。
...
抖动或把半透明变成不透明。编码器发出合法的 literal-only LZW 序列，并正常跨越
9/10/11/12-bit 字典宽度；它尚未用长序列字典引用追求压缩率，但不会按像素反复
clear。实际负荷由 `edge-weight.json` 进入 Planner。
...
单测直接覆盖精确调色板与二值 Alpha 往返、跨越 12-bit 的 8,192 像素 LZW 往返、
多帧不折叠、交错行序、32-bit DIB Alpha/AND mask、重叠成员拒绝、截断前不输出、
Adapter 默认调用和 copy-out 后构建。
...
受控 `ubuntu-24.04`/x86-64 基线已经覆盖全部 133 个生产 Capsule 和 134 条能力：

- GIF 读取：14.298–43.066 MiB/s，峰值内存为输入的 6.647–7.311 倍；
- GIF 精确调色板写入：56.877–136.222 MiB/s，内存为 3.377–3.835 倍；
- ICO/CUR best-member 读取：433.063–1,202.649 MiB/s，内存为 2.146–2.166 倍；
- ICO/CUR 单成员写入：28.520–45.038 MiB/s，内存为 3.751–4.335 倍；
- GIF/ICO/CUR 严格验证：43.840–99.800 MiB/s；
- GIF 动画帧/精灵表渲染：14.272–21.807 MiB/s。

这些向量已写入 29 个新增 Capsule 的 `edge-weight.json`；派生
`load_0_to_100` 范围为 62.816–91.816，原始延迟/内存/输出模型仍是 Planner 的
首要成本输入。
