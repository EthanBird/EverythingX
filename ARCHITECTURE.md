...
凡是暴露 Options 的 Capsule，都必须实现一套可以直接执行的默认参数：

- Rust API 提供 `Default` 或等价构造器；
- `capsule.json` 的 defaults 与代码默认值一致；
- AdapterCapability 显式复制并验证默认映射；
- 默认值不能是空实现、TODO、无效枚举或要求调用者补齐的占位符；
- 面对歧义或不可逆损失，默认策略应确定且保守，优先报错而不是静默丢失；
- 任何默认值变化属于可观察行为变化，必须按 SemVer 和回归测试管理。

Kernel 在注册 Adapter 时验证默认配置完整，CI 使用默认配置至少运行一个有效 fixture。
