...
| Metric | Count |
|---|---:|
| Actually implemented standalone Capsules | 133 |
| Actually implemented Adapter capabilities | 134 |
| Actually implemented logical format pairs | 119 |
...
| Actually implemented distinct-format image pairs | 55 |
| Implemented same-format image validation/transform capabilities | 14 |
| Capsules covered by performance harness | 133 |
| Capability edges covered by performance harness | 134 |
| Capsule-local edge-weight configurations | 133 files covering 134 capabilities |
...
- The batch adds 231 standalone tests and 20 Kernel/Adapter default-invocation tests. Every then-current production Capsule entered recursive copy-out testing and every capability entered the performance harness.

### GIF and ICO/CUR Wave C batch 0.3 — 29 Capsules

- GIF now has twelve directed still-image edges against PNG, BMP, TGA, QOI, PPM and PAM.
- The native GIF decoder covers global/local palettes, LZW dictionary growth, interlace, transparency, loop metadata and disposal methods 0–3.
- Still-image GIF edges reject animation; two explicit animation Capsules render one indexed composited frame or all frames as a horizontal PNG sprite sheet.
- GIF emission is exact-palette only: no silent quantization, partial-alpha removal or palette overflow.
- ICO and CUR each gain six directed best-member/single-member edges against PNG, BMP and QOI.
- ICO/CUR parsing validates directory ranges and supports strict PNG plus uncompressed 1/4/8/24/32-bit DIB, XOR/AND masks and alpha behavior.
- Three validators decode every GIF frame or ICO/CUR member before copying the exact input bytes.
- All 29 crates carry complete native source, tests, runnable defaults and removable Adapters; the controlled CI run measures all 134 capabilities and materializes Capsule-local graph weights.

### Performance evidence 0.2

- One generated release-mode harness recursively covers all 133 production Capsules and all 134 Adapter capabilities.
...
- Every production Capsule now carries a generated root `edge-weight.json`; 133 files contain all 134 capability vectors and an inverse `load_0_to_100` scalar, with profile, harness and environment identity.
...
- The current controlled `ubuntu-24.04`/x86-64 baseline covers 133 Capsules and 134 capabilities: median large-workload throughput is 440.806 MiB/s; observed range is 14.272–3,758.373 MiB/s and calibrated score range is 8.184–58.444.
- Raster Wave A measures 167.451–622.254 MiB/s; its 3.75–4.67 memory ratios make profile-specialized streaming the next evidence-backed optimization target.
- PNG Wave B measures 27.107–85.394 MiB/s. PNG spatial/alpha transforms report a 5.665 input-byte memory ratio, making row streaming and atomic-spool strategies the next measured optimization target.
- GIF reads measure 14.298–43.066 MiB/s and exact-palette writes 56.877–136.222 MiB/s. ICO/CUR best-member reads measure 433.063–1,202.649 MiB/s, single-member writes 28.520–45.038 MiB/s, and explicit GIF animation renderers 14.272–21.807 MiB/s.
...
1. Harden all 133 production Capsules with corpus manifests, fuzz targets and reproducible benchmark reports.
...
3. Implement baseline/progressive JPEG, then Classic TIFF/BigTIFF and WebP.
4. Execute the recorded 20-Capsule native HEIF container wave and 20-Capsule HEIC still-image wave with explicit backend decisions.
5. Extend frame/member algebra to APNG, WebP animation, multi-page TIFF and multi-resolution icon aggregation.
6. Resume the recorded 20-Capsule FLAC family after the requested image phase.
